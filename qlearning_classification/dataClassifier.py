...
def enhancedFeatureExtractorDigit(datum):
    ...
    We used Euler's number to find the number of holes in each datum.
    ##
    """
    ...
    for x in range(1, DIGIT_DATUM_WIDTH - 1):
        for y in range(1, DIGIT_DATUM_HEIGHT - 1):
            if datum.getPixel(x, y) > 0 and datum.getPixel(x - 1, y) == 0 and datum.getPixel(x, y - 1) == 0 and datum.getPixel(x - 1, y - 1) == 0:
                convex += 1
            if datum.getPixel(x, y) == 0 and datum.getPixel(x + 1, y) > 0 and datum.getPixel(x, y + 1) > 0 and datum.getPixel(x + 1, y + 1) > 0:
                concav += 1
    holes = convex - concav
...
def enhancedPacmanFeatures(state, action):
    ...
    "*** YOUR CODE HERE ***"
    #successor = state.generateSuccessor(0, action)
    #distance to each food (inverse) of successor??
    successor = state.generateSuccessor(0, action)
    food = state.getFood()
    height = food.height
    width = food.width
    for x in range(width):
        for y in range(height):
            if successor.hasFood(x, y):
                mhtdist = util.manhattanDistance(successor.getPacmanPosition(), (x, y))
                label = "hasFood at " + str(x) + ", " + str(y)
                features[label] = 1/mhtdist
