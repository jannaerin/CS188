...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
       ...
       # print("noisy is", noisyDistance, "emission is", emissionModel, "pac position is", pacmanPosition)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        counter = particleNum
        while counter > 0:
            particleList += [legalPositions[counter%numLegal]]
            counter -= 1
        if len(particleList) != particleNum:
    ...
    def observe(self, observation, gameState):
        ...
        allPossible = util.Counter()
        for elem, prob in beliefDist.items():
            if noisyDistance is None:
                allPossible[elem] = 0.0
            else:
                trueDistance = util.manhattanDistance(elem, pacmanPosition)
                allPossible[elem] = emissionModel[trueDistance] * beliefDist[elem]
        if noisyDistance is None:
            allPossible[self.getJailPosition()] = 1.0
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        positionIter = itertools.product(legalPositions, repeat=ghostNum)
        positions = list(positionIter)

        
        particleNum = self.numParticles
        numIter = len(positions)
        particleList = []
        counter = particleNum
        while counter > 0:
            particleList += [positions[counter%numIter]]
            counter -= 1
        if len(particleList) != particleNum:
            print("WHAT THE FUCK HAPPENED")
        self.particles = particleList
        ...
        for elem, prob in beliefDist.items():
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] is None:
                    elm = self.getParticleWithGhostInJail(elm, i)
                    continue
                else:
                    curr = util.manhattanDistance(elm[i], pacmanPosition)
                    prob = prob * emissionModels[i][curr]
            allPossible[elm] = prob * beliefDist[elem]
